...
- **2026-04-12 (service consolidation + engagement pipeline)**: Major infrastructure session. (1) Eliminated Formspree — all 12 forms migrated to Netlify Forms with `data-netlify="true"` and `netlify-honeypot="bot-field"`, email notifications wired to info@impactmojo.in via Netlify hooks API. Platform now runs on 2 services (Netlify + Supabase) instead of 3. (2) Applied missing `notifications` + `notification_preferences` migration to Supabase (tables existed in migration file but were never run). Backfilled prefs for all 14 users. (3) Built 5-email engagement drip (Day 0/3/7/14/21) with deduplication via `metadata.drip_stage`. Day 21 pitches Premium with one-time support fallback. (4) Added `update_streak()` DB function called from `auth.js:fetchProfile()`. (5) Created daily Netlify Scheduled Function (08:00 IST) running drip + streak-reminders + cohort-deadlines. (6) Created monthly newsletter function (15th, 10:00 IST) that auto-generates content from `docs/changelog.md` + `search-index.json`. (7) Added Resend.com for transactional email (free tier, 3K/month). Domain verified with DKIM/SPF/DMARC via Netlify DNS API. Key: `re_EjPJtfxt_*` (send-only). (8) Built `/premium-letter.html` (long-form sales letter) and `/starter-kit.html` (10 curated handouts). (9) Branded email template: navy gradient header, stacked logo, blue title bar, amber accent bar, dark footer. (10) Post-certificate email with premium upsell for explorer-tier users. (11) Created test user `connectwithvarna@gmail.com` (password: `ImpactMojo2026!`). (12) Cleaned 20 stale remote branches via GitHub API. (13) 63 drip emails + 9 Day 21 premium emails sent to all users. Supabase usage: 15 MB DB / 15 users / well within free tier.
