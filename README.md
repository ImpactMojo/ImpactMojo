...
|----------|-------------|
| **73 Courses** | 21 flagship + 52 foundational courses across 6 learning tracks (all foundational decks self-hosted as native HTML slide decks). Every flagship ends with an auto-graded "Assess Yourself" self-check |
...
- **[LMS export](https://www.impactmojo.in/lms-export)** (`lms-export.html`) — package any of the 73 courses — plus the 47 practice workbooks — as **SCORM 1.2**, **SCORM 2004**, **IMS Common Cartridge 1.3**, or a single self-contained HTML file. The package is built in your browser from the live page, so it is never stale. Our analytics, sign-in, Supabase and translation code are stripped before packaging and the rest inlined, so an imported course runs with no network and phones nothing home. SCORM reports completion when the learner reaches the final slide.
